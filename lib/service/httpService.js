...
const HTTPService = function (app, opts) {
    ...
    this.address = `${this.protocol === 'http2' ? 'https' : this.protocol}://${this.host}:${this.port}`;
    this.sslOtps = {};
...
HTTPService.prototype.start = function (cb) {
    ...
    if (this.port === 'http2') {
        this.server = http2.createServer(this.sslOtps, this.http.callback()).listen(this.port, this.host);
    } else if (this.protocol === 'https') {
        this.server = https.createServer(this.sslOtps, this.http.callback()).listen(this.port, this.host);
    } else {
        this.server = http.createServer(this.http.callback()).listen(this.port, this.host);
    }

    this.server.on('listening', () => {
        this.state = ST_STARTED;
        this.logger.info('server listening', `url: ${this.address}`);
